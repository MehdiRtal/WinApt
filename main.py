...
if not os.path.exists(folder_name + "packages.json"):
    wget.download(
        "https://raw.githubusercontent.com/MehdiRtal/WinApt/main/packages.json",
        folder_name,
        bar=None)
...
def download_link(arg):
    url = requests.get(
        "https://filehippo.com/download_{}/post_download/".format(arg)).text
    soup = BeautifulSoup(url, "lxml")
    download_link = soup.find("script", {
        "type": "text/javascript",
        "data-qa-download-url": True
    })["data-qa-download-url"]
    if arg == "spotify":
...
def version(arg):
    ...
    version = "".join(
        re.findall(
            "\d|[.]",
            soup.find("p", class_="program-header-inline__version").text))
    return version
...
def install(arg):
    ...
    if data[arg]["installer"] == "innosetup":
        os.system("cmd /c start {} /VERYSILENT /NORESTART".format(
            file_path(arg)))
    ...
    for package in data if args.all or args.list else data and args.package:
        try:
            if "version" not in data:
                ...
                json.dump(data,
                          open(folder_name + "packages.json", "w"),
                          indent=2)
            if args.list:
                print(package + " v" + version(package))
            else:
                if data[package]["version"] == version(
                        package) and os.path.exists(file_path(package)):
                    install(package)
                else:
